...
uint8_t buffer_big[BUFFSIZE_big] __attribute__( ( aligned ( 16 ) ) );
...
//===================================================================================================================
        ...
        if (Pressure_buff_idx == 400) {
            ...
            memcpy(buffer_big, (uint8_t*)acc_buff, 4*ACC_BUFFSIZE);
            memcpy(buffer_big+4*ACC_BUFFSIZE, (uint8_t*)gyr_buff, 4*GYR_BUFFSIZE);
            memcpy(buffer_big+4*ACC_BUFFSIZE+4*GYR_BUFFSIZE, (uint8_t*)mag_buff, 4*MAG_BUFFSIZE);
          //  memcpy(buffer+4*ACC_BUFFSIZE+4*GYR_BUFFSIZE, (uint8_t*)Pressure_buff, 4*Pressure_BUFFSIZE);
            rc = f_write(&fil, buffer_big, BUFFSIZE_big, &wr);
            rc = f_close(&fil);
if (Save_to_SD){
  Pressure_buff_idx=0;
